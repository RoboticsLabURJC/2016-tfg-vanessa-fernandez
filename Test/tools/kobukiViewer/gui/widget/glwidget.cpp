...
void GLWidget::UpdateRobot()
{
    ...
    this->laserData1 = this->robot->getSensors()->getLaserData(1);
    this->laserData2 = this->robot->getSensors()->getLaserData(2);
    this->laserData3 = this->robot->getSensors()->getLaserData(3);
...
void GLWidget::drawLaser()
{
    ...
    for (unsigned k = 0; k < laserData1.size(); k++) {

        Xp_sensor = this->laserData1[k]/100 * cos(((float) k - 90.) * 3.14/180);
        Yp_sensor = this->laserData1[k]/100 * sin(((float) k - 90.) * 3.14/180);
